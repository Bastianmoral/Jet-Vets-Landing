...
export default function ServicioCard({ titulo, icono, className="", lang, showHint=false }) {
  ...
  if (isMobile) {
    return (
      <div className={`relative w-full h-[450px] rounded-2xl shadow-[0_12px_28px_rgba(0,0,0,.18)] bg-[#4E6B3B] text-white p-8 flex flex-col items-center justify-center gap-6 ${className}`}>
        {icono ? <div className="text-7xl">{icono}</div> : null}
        <h3 className="text-2xl font-semibold text-center leading-snug">{titulo}</h3>
        <div className="flex gap-3 mt-2">
          <button
            ...
            className="bg-[#41658A] text-white px-5 py-3 rounded-lg hover:opacity-90 text-base"
          >
            {t.book}
          </button>
          <button
            onClick={(e) => {
              ...
              window.open(`https://wa.me/34666624057?text=${message}`, '_blank');
            }}
            className="bg-[#25D366] text-white px-5 py-3 rounded-lg flex items-center gap-2 hover:opacity-90"
          >
  ...
  // Desktop version - with flip
  return (
    <div
      className={`relative w-full h-[233px] cursor-pointer [perspective:1000px] ${className}`}
